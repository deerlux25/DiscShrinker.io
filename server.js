...
app.get("/health", (req, res) => {
  res.json({
    ok: true,
    service: "discordshrink-compression-server"
  });
});
...
app.post("/compress", upload.single("video"), (req, res) => {
  ...
  ffmpeg.ffprobe(input, (probeError, metadata) => {
    ...
    const videoBitrateKbps = Math.floor(
      (totalBitrate - audioBitrate) / 1000
    );
    ...
    ffmpeg(input)
      ...
      .on("stderr", line => {
        ...
        if (
          /error|fatal|killed|memory|invalid|failed/i.test(message)
        ) {
      ...
      .on("end", () => {
        ...
        if (outputSize > targetBytes) {
          ...
          return res.status(422).json({
            error:
              "The compressed video was larger than the selected target size. Please choose a larger target."
          });
        }
        ...
        res.sendFile(
          output,
          {
            headers: {
              ...
              "Content-Disposition":
                'attachment; filename="compressed-video.mp4"',
...
app.listen(PORT, () => {
  console.log(
    `Compression server running on port ${PORT}`
  );
});
